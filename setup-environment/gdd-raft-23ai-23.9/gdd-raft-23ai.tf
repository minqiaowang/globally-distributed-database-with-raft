...
resource "oci_database_db_system" "shard_db" {
  ...
  db_home {
    database {
      ...
      db_name        = "sdb${count.index}"
      db_unique_name = "sdb${count.index}_${var.db_name_suffix}"
      character_set  = var.character_set
      ncharacter_set = var.n_character_set
      db_workload    = var.db_workload
      pdb_name       = "shard${count.index}"
      db_backup_config {
        auto_backup_enabled = false
      }
    }
    db_version   = "23.9.0.25.07"
    display_name = "sdb${count.index}"
  }
  db_system_options {
  ...
  display_name            = "shardhost${count.index}"
  hostname                = "shardhost${count.index}"
...
resource "oci_database_db_system" "shard2_db" {
  ...
  db_home {
    database {
      admin_password = var.db_admin_password
      db_name        = "sdb2"
      db_unique_name = "sdb2_${var.db_name_suffix}"
      character_set  = var.character_set
      ncharacter_set = var.n_character_set
      db_workload    = var.db_workload
      pdb_name       = "shard2"
...
resource "oci_database_db_system" "shard3_db" {
  ...
  db_home {
    database {
      admin_password = var.db_admin_password
      db_name        = "sdb3"
      db_unique_name = "sdb3_${var.db_name_suffix}"
      character_set  = var.character_set
      ncharacter_set = var.n_character_set
      db_workload    = var.db_workload
      pdb_name       = "shard3"
...
resource "oci_database_db_system" "cata_db" {
  ...
  db_home {
    database {
      admin_password = var.db_admin_password
      db_name        = "catalog"
      db_unique_name = "catalog_${var.db_name_suffix}"
      character_set  = var.character_set
      ncharacter_set = var.n_character_set
      db_workload    = var.db_workload
      pdb_name       = "catapdb"
